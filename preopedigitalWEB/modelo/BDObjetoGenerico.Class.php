...
class BDObjetoGenerico extends BDModeloGenerico {
     ...
     * @throws Exception Posibles errores en la base de datos.
     * @throws Exception Identificador inválido pasado por GET.
     * 
     * @since 01/03/2019
     * @todo 01/03/2019 Tratar inyección de SQL.
     * 
     */
    function buscarPorId($id, $nombreTabla) {

        $this->id = $id ? $id : $this->id;
        $this->query = "SELECT * FROM {$nombreTabla} WHERE id = ? LIMIT 1";

        $this->statement = BDConexion::getInstancia()->prepare($this->query);
        $this->statement->bind_param("i", $id);
        $this->statement->execute();
        $this->datos = $this->statement->get_result()->fetch_assoc();

        if (!$this->datos) {
            throw new Exception("Error en la base de datos. Referencia: ", BDConexion::getInstancia()->errno);
        }

        if (!$this->statement->affected_rows) {
            throw new Exception("Ha ocurrido un error en los parámetros GET POST. Consulte el administrador del sistema. ");
        }

        $this->mapeoDatosObjeto();
    ...
    /**
     * Método genérico para cargar los valores a los atributos del objeto.
     * @since 01/03/2019
     */
    function mapeoDatosObjeto() {
        foreach ($this->datos as $atributo => $valor) {
            $this->{$atributo} = $valor;
        }
    }
    ...
    function setColeccionElementos($tablaVinculacion, $tablaElementos, $idObjetoContenedor, $atributoFKElementoColeccion, $claseElementoColeccion) {
        ...
        for ($x = 0; $x < $this->datos->num_rows; $x++) {
            $this->coleccionElementos[$x] = $this->datos->fetch_object($claseElementoColeccion);
        }
